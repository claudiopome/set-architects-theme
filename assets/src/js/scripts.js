function getCurrentPageStoragePrefix() {
  return window.location.pathname.replace(/\/$/, '');
}

function getPageStorageKey(key) {
...
jQuery(document).ready(function() {
  function setActiveSection(section, clickedElement) {
    const currentActiveSection = jQuery('.content-sections > div.active').hasClass('images-section') ? 'images' :
        jQuery('.content-sections > div.active').hasClass('drawings-section') ? 'drawings' : 'info';
    ...
    if (currentActiveSection === 'images') {
      ...
      if ($sliderImages.hasClass('slick-initialized')) {
        ...
        localStorage.setItem(
            getPageStorageKey('imagesSlideIndex'),
            slick.currentSlide
        );
      }
      ...
      if ($sliderDrawings.hasClass('slick-initialized')) {
        ...
        localStorage.setItem(
            getPageStorageKey('drawingsSlideIndex'),
            slick.currentSlide
        );
      }
    }
    ...
    setTimeout(() => {
      window.requestAnimationFrame(() => {
        if (section === 'images') {
          ...
          if ($sliderImages.hasClass('slick-initialized')) {
            const savedImagesSlide =
                parseInt(
                    localStorage.getItem(getPageStorageKey('imagesSlideIndex')),
                    10
                ) || 0;

            $sliderImages.slick('slickGoTo', savedImagesSlide, true);
          ...
          if ($sliderDrawings.hasClass('slick-initialized')) {
            const savedDrawingsSlide =
                parseInt(
                    localStorage.getItem(getPageStorageKey('drawingsSlideIndex')),
                    10
                ) || 0;

            $sliderDrawings.slick('slickGoTo', savedDrawingsSlide, true);
...
jQuery(document).ready(function () {
  ...
  function updateCounter($slider, $counter) {
    ...
    if (!slick) return;
    const current = slick.slideCount > 1 ? $slider.slick('slickCurrentSlide') + 1 : 1;
    const total = slick.slideCount;
  ...
  jQuery('.prev-images-button, .next-images-button').on('mouseenter', function () {
    $imagesCursor.show();
  }).on('mouseleave', function () {
    $imagesCursor.hide();
  });

  jQuery('.prev-drawings-button, .next-drawings-button').on('mouseenter', function () {
    $drawingsCursor.show();
  }).on('mouseleave', function () {
    $drawingsCursor.hide();
  });
...
jQuery(document).ready(function($) {
  $('.filters-container').each(function() {
    ...
    const $message = $('<p class="no-projects-message" style="display: none;">No matching projects found.</p>');
...
jQuery(document).ready(function($) {
	...
	$('.info-section .projects-header p, .mobile-filters .sort-project').on('click', function() {
		const $this = $(this);
		...
		const sortedProjects = $projects.sort(function(a, b) {
			const aValue = $(a).find('.' + sortKey).text().trim().toLowerCase();
			const bValue = $(b).find('.' + sortKey).text().trim().toLowerCase();
...
jQuery(document).ready(function () {
  ...
  function getSnappedItem($items, $container) {
    ...
    const containerCenter = $container[0].getBoundingClientRect().top + $container.height() / 2;
  ...
  function initializeInteractions() {
    ...
    if (window.innerWidth > 768) {
      ...
      jQuery(".single-news").on("mouseenter", function () {
      ...
      jQuery(".single-press").on("mouseenter", function () {
        const thumbnail = jQuery(this).data("thumbnail");
      ...
      if ($newsContainer.length) {
        useDebouncedScroll($newsContainer, () => {
          const closest = getSnappedItem(jQuery(".single-news"), $newsContainer);
      ...
      if ($pressContainer.length) {
        useDebouncedScroll($pressContainer, () => {
          const closest = getSnappedItem(jQuery(".single-press"), $pressContainer);
          if (closest && closest.length) {
            const thumbnail = closest.data("thumbnail");
            jQuery(".single-press-thumbnail").removeClass("visible");
            jQuery(`#${thumbnail}`).addClass("visible");
...
document.addEventListener('DOMContentLoaded', function () {
    document.cookie = "visited_homepage=true; path=/; max-age=" + 60*60*24*365; // 1 year
});
...
jQuery(document).ready(function(){
  function setOpacity($project, opacity) {
    $project.find('.single-featured-project-visual img').css('opacity', opacity);
  ...
  jQuery('.single-featured-project-visual img, .single-featured-project-visual p, .single-featured-project-info p').hover(
    function() {
